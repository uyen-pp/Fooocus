...
def outpaint_clicked(task: worker.AsyncTask):
...
with shared.gradio_root:
    ...
    with gr.Row():
        with gr.Column(scale=2):
            ...
            with gr.Row():
                ...
                with gr.Column(scale=3, min_width=0):
                    ...
                    outpaint_button = gr.Button(label="Outpainting", value="Outpainting", elem_classes='type_row', elem_id='outpaint_button', visible=False)
        ...
        with gr.Column(scale=1, visible=modules.config.default_advanced_checkbox) as advanced_column:
            with gr.Tab(label='Settings'):
                ...
                def update_history_link():
                    ...
                    return gr.update(value=f'<a href="file={get_current_html_path(output_format)}" target="_blank">\U0001F4DA History Log</a>') # fn_index = 70
        ...
        outpaint_button.click(lambda: True, outputs=state_is_generating) \
            .then(fn=refresh_seed, inputs=[seed_random, image_seed], outputs=image_seed) \
            .then(fn=get_task, inputs=ctrls, outputs=currentTask) \
            .then(fn=outpaint_clicked, inputs=currentTask, outputs=gallery) \
            .then(lambda: False, outputs=state_is_generating)
